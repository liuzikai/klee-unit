...
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  // Add watched objs
  ConstraintSet testCaseConstraints =
      Assignment(objects, values).createConstraintsFromAssignment();
  for (const auto &obj : state.watchedObjs) {

    // Compute values
    std::vector<unsigned char> value;

    for (unsigned i = 0; i < obj.first->size; ++i) {
      ref<Expr> e = obj.second->read8(i);
      ref<ConstantExpr> result;
      success =
          solver->getValue(testCaseConstraints, e, result, state.queryMetaData);
      if (!success) {
        klee_warning(
      ...
      bool mustBeTrue;
      success =
          solver->mustBeTrue(testCaseConstraints, EqExpr::create(e, result),
                             mustBeTrue, state.queryMetaData);
      if (!success) {
        klee_warning(
            "unable to compute initial values (invalid constraints?)!");
      ...
      if (!mustBeTrue) {
        klee_warning("watched object %s does not evaluate to a unique value (why?)",
                     obj.first->name.c_str());
      }

      value.emplace_back((unsigned char)result->getZExtValue(8));
    }

    res.push_back(std::make_pair(obj.first->name, value));
  }
