...
/* caches */
Cache *inst_cache = NULL;
Cache *data_cache = NULL;

#ifndef INST_CACHE_WAY
#define INST_CACHE_WAY 4
#endif

#ifndef INST_CACHE_SET
#define INST_CACHE_SET 64
#endif

#ifndef INST_CACHE_BLOCK
#define INST_CACHE_BLOCK 32
#endif

#ifndef INST_CACHE_POLICY
#define INST_CACHE_POLICY LRU
#endif

#ifndef DATA_CACHE_WAY
#define DATA_CACHE_WAY 8
#endif

#ifndef DATA_CACHE_SET
#define DATA_CACHE_SET 256
#endif

#ifndef DATA_CACHE_BLOCK
#define DATA_CACHE_BLOCK 32
#endif

#ifndef DATA_CACHE_POLICY
#define DATA_CACHE_POLICY LRU
#endif

#define VALUE_TO_STRING(x) #x
#define VALUE(x) VALUE_TO_STRING(x)
//#pragma message("Inst cache: " VALUE(INST_CACHE_WAY) " ways, " VALUE(INST_CACHE_SET) " sets, " VALUE(INST_CACHE_POLICY))
//#pragma message("Data cache: " VALUE(DATA_CACHE_WAY) " ways, " VALUE(DATA_CACHE_SET) " sets, " VALUE(DATA_CACHE_POLICY))

void pipe_init()
{
    memset(&pipe, 0, sizeof(Pipe_State));
    pipe.PC = 0x00400000;
    pipe.inst_cache_stall = -1;  /* inst not ready */
    pipe.data_cache_stall = -1;  /* data not ready */

    inst_cache = cache_create(INST_CACHE_WAY, INST_CACHE_SET, INST_CACHE_BLOCK, INST_CACHE_POLICY);
    data_cache = cache_create(DATA_CACHE_WAY, DATA_CACHE_SET, DATA_CACHE_BLOCK, DATA_CACHE_POLICY);
}

void pipe_deinit()
{
    if (inst_cache) cache_destroy(inst_cache);
...
void pipe_cycle()
{
    ...
    if (pipe.branch_recover) {
        ...
        pipe.inst_cache_stall = -1;  /* inst not ready (cancel the current operation if any) */
        ...
        if (pipe.branch_flush >= 3) {
            ...
            pipe.multiplier_stall = 0;
        }

        if (pipe.branch_flush >= 4) {
            if (pipe.mem_op) free(pipe.mem_op);
            pipe.mem_op = NULL;
            pipe.data_cache_stall = -1;  /* data not ready (cancel the current operation if any) */
        }
...
void pipe_stage_wb()
{
    ...
    if (op->opcode == OP_SPECIAL && op->subop == SUBOP_SYSCALL) {
        if (op->reg_src1_value == 0xA) {
            ...
            pipe.inst_cache_stall = 0;  /* let fetch do pc + 4 without waiting */
            RUN_BIT = 0;
        }
    }
...
void pipe_stage_mem()
{
    ...
    if (op->is_mem) {
        ...
        /* if we are waiting for the data cache, return */
        if (pipe.data_cache_stall == -1) {
            pipe.data_cache_stall = cache_test(data_cache, op->mem_addr & ~3);
        }

        if (pipe.data_cache_stall) {
    ...
    uint32_t val = 0;
    if (op->is_mem) {
        /* we discard the return val as it's handled with cache_test() */
        cache_read_32(data_cache, op->mem_addr & ~3, &val);
    }
    ...
    switch (op->opcode) {
        ...
        case OP_SB:
            ...
            cache_write_32(data_cache, op->mem_addr & ~3, val);
            break;
...
{
    /* if PC is not in the cache, we need to wait */
    if (pipe.inst_cache_stall == -1) {
        pipe.inst_cache_stall = cache_test(inst_cache, pipe.PC);
    }

    /* if we are waiting for the inst cache, return */
    ...
    /* we discard the return val as it's handled with cache_test() */
    cache_read_32(inst_cache, pipe.PC, &op->instruction);
