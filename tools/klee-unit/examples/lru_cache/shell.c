...
/*                                                            */
/**************************************************************/
void load_program(char *program_filename, uint32_t start) {
  FILE * prog;
  int ii, word;
...
void initialize(char *program_filename, int num_prog_files) { 
  ...
  init_memory();
  pipe_init();
  load_program(program_filename, MEM_TEXT_START);
  while(*program_filename++ != '\0');
  if (num_prog_files == 2) load_program(program_filename, MEM_DATA_START);
...
int main(int argc, char *argv[]) {                              
  ...
  /* Error Checking */
  if (argc < 2 || argc > 3) {
    printf("Error: usage: %s text_file <data_file>\n",
           argv[0]);
    pipe_deinit();
    exit(1);
  }
