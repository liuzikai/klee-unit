...
//   Cache *cache = ;
//   uint32_t address;
//   klee_make_symbolic(&address, sizeof(uint32_t), "address");
//   uint32_t value_ptr;
//   int ret = cache_read_32(cache, address, &value_ptr);
//   klee_watch_obj(&ret, "ret");
//   klee_watch_obj(&value_ptr, "value_ptr");
//   cache_destroy(cache);
// }
...
//  klee_make_symbolic(&address, sizeof(uint32_t), "address");
//  uint32_t value;
//  klee_make_symbolic(&value, sizeof(uint32_t), "value");
//  int ret = cache_write_32(cache, address, value);
//  klee_watch_obj(&ret, "ret");
//}
...
void klee_unit_test_cache_read_32()
{
  ...
  uint32_t value_ptr;
  int ret = cache_read_32(cache, address, &value);
  WATCH(value);
  WATCH(ret);
}
